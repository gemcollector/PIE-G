...
def make_mani_env(
	...
	env = FrameStack(env, frame_stack)
	# if not is_distracting_cs:
	# 	env = MANIColorWrapper(env, mode, seed)
...
class MANIColorWrapper(gym.Wrapper):
	"""Wrapper for the color experiments"""
	...
	def _get_dmc_wrapper(self):
		_env = self.env
		while not isinstance(_env, dmc2gym.wrappers.MANIWrapper) and hasattr(_env, 'env'):
			_env = _env.env
		assert isinstance(_env, dmc2gym.wrappers.MANIWrapper), 'environment is not dmc2gym-wrapped'
